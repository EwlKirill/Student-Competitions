...
| IV | Role-Based Access & Data Scoping (NON-NEGOTIABLE) | **PASS (no protected routes exist)** | There are no roles or users yet (milestones 4–5). The only data surface is the **anonymous, read-only** home page, which by design shows only question text; reference answers never reach the template (`QuestionPublic`). Deny-by-default is honoured in the strongest form available before auth exists: **no route accepts a write method at all**, asserted by `test_routes.py` over the whole route table (FR-018). The service functions carry no authorization yet because they have no caller besides tests. Milestone 6 must add role checks where it calls them. That obligation is recorded in [contracts/question-service.md](./contracts/question-service.md) by stating the interface is not exposed. |
...
| — | Development Workflow gate 3 (PR gates) | **PASS** | CI is still green-or-blocked on the same two contexts. The new tables arrive with migrations. No new route, so no new role-access tests are required, and the "no write routes" test covers the surface. No secrets in the diff (V10). README updated in the same PR (FR-032, Workflow §5). |
...
**Post-Phase 1 re-check**: **PASS**. No verdict changed after writing
...
introduced exactly the three dependencies above, one engine-specific statement (justified below),
and no write route. Two design details strengthened principles rather than bending them:
...
There are no constitution violations. One engine-specific feature and the residual manual
verification are recorded here, because Governance requires them to be visible.
